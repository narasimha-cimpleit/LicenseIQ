...
- **Quick Assign to Organization UX** (2025-11-21): Added streamlined "Quick Assign" button directly on each user row (blue UserPlus icon) that opens a one-click dialog for fast organization assignments. Replaces the old multi-step workflow (expand row → Organizations tab → Add Assignment) with immediate access to assignment form. Dialog features cascading dropdowns, proper query invalidation for instant UI updates, and works alongside existing Organizations tab for viewing/managing all assignments.
