...
- **Inline Quick Assign UX** (2025-11-21): Replaced dialog-based Quick Assign with inline expander for seamless admin workflow. Clicking the blue UserPlus icon expands the user row to reveal a compact 4-column assignment form (Company → Business Unit → Location → Role) right in the table. Features: dedicated state management separate from Organizations tab, allows multiple consecutive assignments without losing context, auto-resets form fields after submission while keeping panel open, proper query invalidation for instant UI updates. Users can manually switch to Organizations tab to review all assignments when desired. This inline approach eliminates modal disruptions common in data-dense admin consoles.
- **User-Organization Role Management** (2025-11-21): Implemented complete system for assigning users to organizations with role-based access control. Users can now be assigned to multiple companies, business units, and locations with specific roles (owner, admin, editor, viewer, auditor) at each level. Features include: dedicated Organizations tab in User Management page, cascading dropdowns for company → business unit → location selection, visual assignment cards showing full hierarchy path, and CRUD operations for managing assignments. Backend includes secure API endpoints (admin/owner only), proper foreign key relationships in user_organization_roles table, unique constraint preventing duplicate role assignments for same location, and full audit trail integration.
