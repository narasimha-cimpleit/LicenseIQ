...
- **LicenseIQ Schema Catalog** (November 6, 2025): **Manageable LicenseIQ standard schema system** mirroring ERP Catalog architecture for platform-wide data consistency. Database architecture includes two core tables: `licenseiq_entities` (entity definitions with categories) and `licenseiq_fields` (field definitions with data types, validation, required flags). **Schema Catalog Management UI** provides comprehensive CRUD operations with tabbed interface (Entities/Fields), category filtering (Master Data/Transactional/Rules), search capabilities, and professional dialogs. **Auto-seeded with 5 standard entities**: Products (10 fields matching TESTING_GUIDE.md: productCode, productName, category, territory, quantity, grossAmount, transactionDate, transactionId, unitPrice, discount), Sales Data (12 fields), Contract Terms (10 fields), Royalty Rules (10 fields), Payments (10 fields). **Seed script** (`server/seed-licenseiq-schema.ts`) runs automatically on server startup, ensuring consistent schema across deployments. Storage layer implements 10 CRUD methods (createLicenseiqEntity, getLicenseiqEntity, updateLicenseiqEntity, deleteLicenseiqEntity, getAllLicenseiqEntities, createLicenseiqField, getLicenseiqField, updateLicenseiqField, deleteLicenseiqField, getLicenseiqFieldsByEntity). Backend provides 8 authenticated API endpoints with validation. Accessible via sidebar navigation with Layers icon.
- **AI Master Data Mapping** (November 2025, Enhanced November 6, 2025): AI-driven field mapping system with **universal ERP support** and **dual schema auto-population** via dynamic catalog integration. Features include: **Groq LLaMA-powered intelligent mapping** with semantic field matching, confidence scoring (High ≥90%, Medium 70-89%, Low <70%), and transformation rule suggestions; **dual-dropdown schema selection** - ERP entity selector auto-populates SOURCE schema, LicenseIQ entity selector auto-populates TARGET schema (replaces manual JSON entry); **dynamic ERP/entity selection** from catalog using dropdown menus; **auto-populated schema JSON** with field count indicators and green checkmarks; **complete CRUD operations** for mapping configurations with save, load, view, edit, and delete functionality; **mapping results table** displaying source-to-target field mappings with confidence badges and transformation rules; **export capability** for JSON mapping files; **tabbed interface** with "Generate Mapping" and "Saved Mappings" views; **full type safety** with TypeScript interfaces for FieldMapping, MappingResult, and SavedMapping; stored in master_data_mappings table with JSONB mapping results for flexible schema storage; accessible via sidebar navigation with Database icon; includes "Configure ERP Catalog" and "Configure Schema" buttons linking to respective catalog pages.
