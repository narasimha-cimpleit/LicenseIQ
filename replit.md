...
- **Admin Page Layout Standardization** (2025-11-24): Completed conversion of 7 admin pages to use MainLayout component for consistent sidebar navigation across the application. Updated pages: master-data-mapping.tsx, erp-catalog.tsx, licenseiq-schema.tsx, master-data.tsx, admin-leads.tsx, configuration.tsx, and data-management.tsx. Removed custom headers, back buttons, and breadcrumbs in favor of unified navigation experience via persistent sidebar. Enhanced MainLayout and Header components with optional `actions` prop to support custom action buttons in the header. Master-data.tsx and data-management.tsx use the actions prop to display page-specific controls (status filter, add company button, seed buttons). All pages verified working correctly with workflow running successfully.
