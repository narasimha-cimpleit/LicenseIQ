...
- **User-Organization Role Management** (2025-11-21): Implemented complete system for assigning users to organizations with role-based access control. Users can now be assigned to multiple companies, business units, and locations with specific roles (owner, admin, editor, viewer, auditor) at each level. Features include: dedicated Organizations tab in User Management page, cascading dropdowns for company → business unit → location selection, visual assignment cards showing full hierarchy path, and CRUD operations for managing assignments. Backend includes secure API endpoints (admin/owner only), proper foreign key relationships in user_organization_roles table, and full audit trail integration.
